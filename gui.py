...
class GUI():
    ...
    def __init__(self):
        ...
        self._start_btn = tk.Button(self._root, text="START", command=self.timer_start, width=6, height=2)
        self._start_btn.place(x=5, y=100)

        self._stop_btn = tk.Button(self._root, text="STOP", command=self.timer_stop, width=6, height=2)
        self._stop_btn.place(x=105, y=100)

        self._reset_btn = tk.Button(self._root, text="RESET", command=self.timer_reset, width=6, height=2)
        self._reset_btn.place(x=205, y=100)

        self._increase_hours_btn = tk.Button(self._root, text="+", command=self.increase_hours)
        self._increase_hours_btn.place(x=40, y=0)

        self._decrease_hours_btn = tk.Button(self._root, text="-", command=self.decrease_hours)
        self._decrease_hours_btn.place(x=40, y=70)

        self._increase_minutes_btn = tk.Button(self._root, text="+", command=self.increase_minutes)
        self._increase_minutes_btn.place(x=125, y=0)

        self._decrease_minutes_btn = tk.Button(self._root, text="-", command=self.decrease_minutes)
        self._decrease_minutes_btn.place(x=125, y=70)

        self._increase_seconds_btn = tk.Button(self._root, text="+", command=self.increase_seconds)
        self._increase_seconds_btn.place(x=210, y=0)

        self._decrease_seconds_btn = tk.Button(self._root, text="-", command=self.decrease_seconds)
        self._decrease_seconds_btn.place(x=210, y=70)
